...
async function renderDatasetsPage() {
                ...
                <article class="run-summary-card">
                  ...
                  <form id="launchRunForm">
                    ...
                    <div class="form-group" style="margin-top:8px;">
                      ...
                      <div class="form-group" style="margin-bottom:0;">
                        <label class="label-with-hint" for="max_concurrent_trials">
                          <span>Максимум параллельных обучений (max_concurrent_trials)</span>
                          <button
                            type="button"
                            class="field-hint-icon"
                            title="Внимание: Увеличение этого параметра ускоряет поиск, но требует больше видеопамяти. При ошибках Out of Memory уменьшите значение"
                            aria-label="Внимание: Увеличение этого параметра ускоряет поиск, но требует больше видеопамяти. При ошибках Out of Memory уменьшите значение"
                          >?</button>
                        </label>
                        <input
                          type="number"
  ...
  try {
    ...
    const mcRaw = payload.max_concurrent_trials;
    ...
    if (
      ...
      throw new Error("Укажите целое число max_concurrent_trials не меньше 1.");
    }
    payload.max_concurrent_trials = mc;

    appendTrainingMonitorLog(trainingMonitorLine(`Бюджет попыток (n_trials): ${nTrials}`));
    appendTrainingMonitorLog(
      trainingMonitorLine(`Максимум параллельных обучений: ${mc}`)
    );
    appendTrainingMonitorLog(trainingMonitorLine("Отправка запроса на запуск обучения"));
          ...
          <div class="table-wrap">
            <table class="table">
              <tbody>
                ...
                <tr><th>Макс. параллельных обучений</th><td>${detail.max_concurrent_trials != null ? escapeHtml(String(detail.max_concurrent_trials)) : "—"}</td></tr>
